...
extension CollectionViewController: CLLocationManagerDelegate {
    ...
    func locationManager(_ manager: CLLocationManager, didChangeAuthorization status: CLAuthorizationStatus) {
        ...
        case .denied, .restricted:
             ...
             viewModel.fetchCities(keyword: nil, lat: "43.6532", long: "-79.3832")
        case .authorizedWhenInUse:
            guard let locValue: CLLocationCoordinate2D = manager.location?.coordinate else { return }
            print("Got Location")
            print("Fetching Restaurants")
            viewModel.fetchCities(keyword: nil, lat: String(locValue.latitude), long: String(locValue.longitude))
        default:
            break
        }
    }
...
extension CollectionViewController: UISearchBarDelegate {
    ...
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        if let locValue: CLLocationCoordinate2D = locationManager?.location?.coordinate {
            viewModel.fetchCities(keyword: searchBar.text, lat: String(locValue.latitude), long:  String(locValue.longitude))
        } else {
            //Default to Toronto
            viewModel.fetchCities(keyword: searchBar.text, lat: "43.6532", long: "-79.3832")
        }
